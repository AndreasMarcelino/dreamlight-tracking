  ...
  return (
    <div className="space-y-8">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className={`w-14 h-14 rounded-xl ${card.bgColor} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                <i className={`fa-solid ${card.icon} text-2xl ${card.textColor}`}></i>
              </div>
              <div className={`px-2 py-1 rounded-lg bg-gradient-to-r ${card.gradient} bg-opacity-10`}>
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Your latest production updates</p>
            </div>
            <Link
              to="/projects"
              ...
              {stats.recentProjects && stats.recentProjects.length > 0 ? (
                stats.recentProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-gray-50 transition group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(project.global_status)}`}>
                        {project.global_status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1.5 w-48">
                        {project.progress_stats && Object.entries(project.progress_stats).map(([phase, percentage]) => (
                          <div key={phase} className="flex items-center gap-2">
                            <span className="text-[9px] text-gray-400 font-bold uppercase w-12 shrink-0">
                              {phase === 'Pre-Production' ? 'Pre' : phase === 'Production' ? 'Prod' : 'Post'}
                            </span>
                            <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${
                                  phase === 'Pre-Production' ? 'bg-indigo-500' :
                                  phase === 'Production' ? 'bg-orange-500' : 'bg-blue-500'
                                }`}
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        ...
                        className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-200 hover:border-indigo-600 hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 transition"
                      >
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400">
                      ...
                      <Link
                        to="/projects/create"
                        className="mt-4 text-indigo-600 hover:underline font-semibold text-sm"
                      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Link
          to="/projects/create"
          className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-6 text-white hover:shadow-2xl hover:scale-105 transition-all group"
        >
          ...
          <p className="text-indigo-100 text-sm">Start tracking a new production</p>
        </Link>
