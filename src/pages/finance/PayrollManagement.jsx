...
export default function PayrollManagement() {
  ...
  const fetchPendingPayrolls = async () => {
    ...
    try {
      ...
      hasShownError.current = true; // Mark that error has been shown

      const errorMessage = error.response?.data?.message || error.message;

      if (errorMessage.includes("User is not associated")) {
        toast.error(
          "Backend error: User model not properly associated. Please check backend models.",
        );
      } else {
        toast.error(`Failed to load pending payrolls: ${errorMessage}`);
      }

      // Set empty state only if component is still mounted
    ...
    if (result.isConfirmed && isMounted.current) {
      try {
        ...
        for (const id of selectedIds) {
          if (!isMounted.current) break; // Check mounted state during loop
          await financeService.payCrew(id);
        }

        if (isMounted.current) {
          toast.success(
            `${selectedIds.length} payments processed successfully!`,
          );
          fetchPendingPayrolls();
          setSelectedIds([]);
        }
        ...
        if (isMounted.current) {
          toast.error(error.response?.data?.message || "Batch payment failed");
        }
        console.error(error);
      }
    }
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-6">
            <div>
              ...
              <p className="text-orange-100">
                Process crew payments efficiently
              </p>
            </div>
          ...
          <div className="p-16 text-center">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-500">
                No pending crew payments at the moment
              </p>
            </div>
          </div>
        )}
